"""
Long-Term Memory - Vector-based semantic memory using pgvector.
...
Uses the existing VectorStore and EmbeddingService from context module.
"""
import uuid
...
class LongTermMemory:
    ...
    Wraps the existing VectorStore to provide a clean interface
    ...
    async def store_plan(
        ...
        Creates embeddings for plan summary and weekly focuses.
        
        Args:
            plan_id: Plan UUID
            plan_data: Full plan data including weeks
        """
        ...
        logger.info(
            "Stored plan in long-term memory",
            plan_id=str(plan_id),
    ...
    async def store_analysis(
        ...
        logger.debug("Stored analysis in long-term memory", plan_id=str(plan_id))
    ...
    async def store_conversation(
        ...
        logger.debug("Stored conversation in long-term memory", plan_id=str(plan_id))
    ...
    def _create_plan_summary(self, plan_data: dict[str, Any]) -> str:
        """Create text summary of training plan."""
        user_profile = plan_data.get("userProfile", {})
        weeks = plan_data.get("weeks", [])
    ...
    def _create_week_summary(self, week: dict[str, Any]) -> str:
        """Create text summary of a training week."""
        parts = [
