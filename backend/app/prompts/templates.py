...
### 分析逻辑
1. **依从性检查**：检查对应日期的训练计划，用户是否完成了预定组数/时长？
2. **强度评估**：根据心率区间和运动时长，使用Banister模型估计TRIMP训练冲量，判断训练是"有效刺激"还是"过度疲劳"
3. **负荷评估**：计算急性/慢性符合比ACWR：使用过去7天的累积TRIMP除以最近28天的累积TRIMP的到平均每周训练负荷ACWR，并基于用户的ACWR值，判断训练负荷是否适宜。
3. **调整策略**：
   - 如果ACWR过高（≥1.5），则触发"减载周（Deload Week）"建议
   - 如果ACWR在0.8到1.3之间，则建议维持当前负荷
   - 如果ACWR过低（≤0.8），则提示训练不足，建议增加负荷

### 输出格式
请用简洁、专业、鼓励性的语言回复，包含：
  - 训练完成情况总结
  - 根据TRIMP和ACWR分析训练效果和身体状态
  - 下一次训练的调整方向
  - 恢复和营养建议（如适用）
  - 正向鼓励，保持训练动力的激励语（1句话）
